...
Create your own culture texts for the Syncfusion User Management Server and add it in the application anytime.

To make Syncfusion User Management Server use a different culture perform the following steps:
...
3. Upload [default.po](locale/default.po) file which contains the texts from the User Management Server application from `~Installed Location~/User Management Server/UMS.Web/locale into google translator`.
...
9. Create a folder in `~Installed Location~/User Management Server/UMS.Web/locale` with {language code}-{country code} and paste the downloaded messages.po file inside the newly created folder.

    For example, if you are translating to Italian, create a folder named `it-it` and paste the messages.po like the below.
    
    `~Installed Location~/User Management Server/UMS.Web/locale/it-it/messages.po`
    
    Newly added language will be listed in language list in User Management Server after refreshing the page in the User Management Server.
